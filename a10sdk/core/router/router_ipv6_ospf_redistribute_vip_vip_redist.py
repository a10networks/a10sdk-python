...
class VipRedist(A10BaseClass):
    ...
    :param vip_list: {"minItems": 1, "items": {"type": "object"}, "uniqueItems": true, "type": "array", "array": [{"properties": {"metric-vip": {"description": "OSPF default metric. help-val OSPF metric", "minimum": 0, "type": "number", "maximum": 16777214, "format": "number"}, "metric-type-vip": {"default": "2", "enum": ["1", "2"], "type": "string", "description": "1: \"Set OSPF External Type 1 metrics\"; 2: \"Set OSPF External Type 2 metrics\"; ", "format": "enum"}, "optional": true, "type-vip": {"enum": ["only-flagged", "only-not-flagged"], "type": "string", "description": "only-flagged: \"Selected Virtual IP (VIP)\"; only-not-flagged: \"Only not flagged\"; ", "format": "enum"}, "route-map-vip": {"minLength": 1, "maxLength": 128, "type": "string", "description": "Route map reference. help-val Pointer to route-map entries", "format": "string"}}}]}
    :param DeviceProxy: The device proxy for REST operations and session handling. Refer to `common/device_proxy.py`

    

    URL for this object::
    `https://<Hostname|Ip address>//axapi/v3/router/ipv6/ospf/{process_id}/redistribute/vip/vip-redist`.
    ...
    def __init__(self, **kwargs):
        ...
        self.a10_url="/axapi/v3/router/ipv6/ospf/{process_id}/redistribute/vip/vip-redist"
        self.DeviceProxy = ""
        self.vip_list = []
